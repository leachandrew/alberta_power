...
if(update!=1){
  ...
  renew_vols<- all_vols %>% #filter(year(date)>=2016) %>%
  ...
  test<-merit_aug %>% filter(block_number==2,renew_gen>0)
  ...
  print(paste("Filled Climate Policy. Elapsed time is",time_length(interval(start_time, Sys.time()), "seconds"),"seconds"))
    ...
    ungroup() %>%arrange(date,he,merit) %>% select(date,he,asset_id,Plant_Type,block_number,price,size,flexible,available_mw,dispatched_mw,merit,co2_est,oba_val,ctax_cost,net,import_export)
  
  
  #need to go back and figure this out - why was this here?
...
save(merit_aug,file="merit_aug_blocks.RData")   
paste("Built Albert's data, elapsed time is",time_length(interval(start_time, Sys.time()), "seconds"),"seconds")
...
ggplot(brooks_test) + geom_line(aes(hour,gen_mean))+
...
gloria_data<-merit_aug %>% select(date,he,asset_id,Plant_Type,block_number,size,flexible,price,import_export,available_mw,dispatched_mw,merit,actual_posted_pool_price,actual_ail,month,day,hour,year)
