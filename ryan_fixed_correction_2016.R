...
#This section calculates the supply cushion for each hour
...
if  (input_fuel == "NGcogenAB_oilsands") {
  ...
  high_shape$quartile <- rep(c('.25', '.50', '.75', '.90', '.95', '.99'), times = nrow(high_shape)/6)
  ...
  for(quar in unique(high_shape$quartile)){
    ...
    select_per$month <- rep(c(1:12), times = nrow(select_per)/12)
  ...
  low_shape$quartile <- rep(c('.25', '.50', '.75', '.90', '.95', '.99'), times = nrow(low_shape)/6)
...
if  (input_fuel == "CoalWCA") {
  ...
  low_power_shape$quartile <- rep(c('.25', '.50', '.75', '.90', '.95', '.99'), times = nrow(low_power_shape)/6)
