...
price_model <- lm(actual_posted_pool_price ~factor(he) + factor(day)+factor(month)+factor(year)+factor(on_peak), data = data_set)
tidy(price_model)
resid<-augment(price_model)

resid<-resid%>% mutate(resid=actual_posted_pool_price-.fitted,
                       date=ymd_h(paste(`factor(year)`,"-",`factor(month)`,"-",`factor(day)`," ",`factor(he)`,sep="")))
