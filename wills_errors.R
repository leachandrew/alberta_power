...
# Choose reduced model 14
coeffs<-tidy(red_model14)
resid<-augment(red_model14)
...
ts_test<-as_tibble(rnorm(nrow(resid)+500,0,sd=sum14$sigma)) #use 500 iterations to seed the processs
...
comb.ts <- cbind(resid$actual_posted_pool_price,resid$.fitted,ts_test$ar_val)
