...
test<-merit_combine(merit_AS_data,forecast_data,start_date=as.Date("2013-01-01"),end_date=as.Date("2017-12-31"))
test<-test %>% mutate(Month=as.numeric(month(time)),Day=as.numeric(day(time)),`Hour Ending`=as.numeric(ifelse(hour(time)==0,24,hour(time))))
...
missing<-ELS_data %>% filter(is.na(PVSyst))
ELS_data<-ELS_data %>% filter(!is.na(PVSyst))
...
#for(batt_size in c(seq(0,15000,by=15000))){  #if you're going to comment this in, close the bracket at the end
  batt_size<-15000 #15MWh Battery
  batt_in<- 6500  #max inflow
  batt_out<-6500  #max outflow
  ...
  ELS_data$net_imp<-0
    ...
    #ELS_data <- ELS_data %>% mutate(outflow_cons=ifelse(batt_pwrbatt_out,batt_call-batt_out,0),
...
hours<-tightest_hours() %>% filter(local_date >= min(ELS_data$date) & local_date<= max(ELS_data$date))
ELS_data <- ELS_data %>% mutate(tight_hour=ifelse(time %in% hours$local_date,1,0))

write.xlsx(ELS_data, file = paste("ELS_battery_size_",batt_size,".xlsx",sep=""), colNames = TRUE, borders = "columns")
