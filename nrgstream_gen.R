#code to load and process NRGStream hourly gen data
...
all_gen_data <- function() {
  ...
  for (y_data in years){
  #testing y_data<-2004
    file_name<-paste("nrgstream/hourly",sprintf('%02d', y_data %% 100),".csv",sep="")
    ...
    new_melt <- new_data%>%pivot_longer(cols=-c("Time","Demand","Price","AIL"),values_to="gen",names_to="NRG_Stream") #  melt(new_data,id=,variable.name="NRG_Stream",value.name = "gen") 
...
data_update <- function(data_sent,fix_year=0) {
  ...
  #new_melt$Time_TEST<-parse_date_time(new_melt$Time, orders = c("ymd_HM", "dmy_HM", "mdy_HM","dby_HM","dby_HMS"),tz="America/Denver")
  
  new_melt<-new_melt %>% mutate(Time=parse_date_time(Time, orders = c("ymd_HM", "dmy_HM", "mdy_HM","dby_HM","dby_HMS"),tz="America/Denver"))%>%
    filter(!is.na(Time))
  #convert trade so that imports are positive, exports negative so same sign as gen
...
nrgstream_gen<-nrgstream_gen %>% 
  mutate(Plant_Type=case_when( 
    (ID=="HRM") & (date>=ymd("2020-05-08")) ~ "SCGT",  #Milner change to gas effective 
    ...
    (ID %in% c("SH1","SH2")) & (date>=ymd("2021-07-30")) ~ "NGCONV",
    #KH3 January 11, 2022
    (ID =="KH1") & (date>=ymd("2022-01-11")) ~ "NGCONV",
    #KH2 July 27, 2021.
    (ID =="KH2") & (date>=ymd("2021-07-21")) ~ "NGCONV",
    #Battle River #4 (BR4)	March 8, 2022
    (ID =="BR4") & (date>=ymd("2022-03-08")) ~ "NGCONV",
    #Battle River #5 (BR5)	 November 19, 2021
    (ID =="BR5") & (date>=ymd("2021-11-19")) ~ "NGCONV",
    #Sundance #6 (SD6)	401	0	0 February 19, 2021
    (ID =="SD6") & (date>=ymd("2021-02-19")) ~ "NGCONV",
    TRUE ~ Plant_Type),
    Capacity=case_when(
      (ID=="HRM") & (date>=ymd("2020-04-23"))&(date<ymd("2020-05-08")) ~ 185,  #Milner change to gas effective 
      (ID=="HRM") & (date>=ymd("2020-05-08"))&(date<ymd("2021-12-09")) ~ 208,  #Milner change to gas effective 
      (ID=="HRM") & (date>=ymd("2021-12-09")) ~ 300,  #Milner change to gas effective 
      TRUE~Capacity
    )
