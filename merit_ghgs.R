...
#setwd("G:/My Drive/alberta_power")
...
ghg_merit<-function(){
  ...
  #merit_data<-merit_data %>% filter(date==ymd("2009-09-16"),he=="18")
  ...
  #now, we need to take renewables out of merits, and then add in info from renewable_vols
  
  #checked and the effective times are odd
  #dates<-merit_aug %>% group_by(date,he) %>% summarize()
  
  renew_vols <- renew_vols %>% filter(date>=min(merit_aug$date))%>%
    mutate(
    import_export="",
    block_number=0,
    price=0,
    ...
    to=renew_gen,
    size=renew_gen,
    available_mw=renew_gen,
    dispatched="Y",
    dispatched_mw=renew_gen,
    flexible="N",
    offer_control="",
    key_firm=(1==2),
    offer_sum=as.factor("Other")
  ) 
                                         
  #renew_vols<-renew_vols %>% filter(date==ymd("2009-09-16"),he=="18")
  
  merit_aug<-merit_aug%>% filter(!asset_id %in% unique(renew_vols$asset_id)) %>% select(-effective_date_time) %>%
    bind_rows(renew_vols)%>% group_by(date,he) %>% arrange(price)%>%   mutate(
      merit=cumsum(size)
    ) 
  ...
  merit_aug<-merit_aug %>% #filter(year(date)>=2012) %>%
    ...
    ungroup() %>%arrange(date,he,merit) %>% select(date,he,asset_id,Plant_Type,offer_sum,block_number,price,from,to,size,available_mw,dispatched_mw,merit,co2_est,oba_val,ctax_cost,net,import_export,block_ghgs,merit_ghgs)
  ...
  merit_aug <- merit_aug %>% assign_time() %>% assign_date_time_days()
  ...
  write.csv(merit_aug,"gloria.csv")
...
merit_test<-merit_aug %>% filter(date==ymd("2012-01-01"),he=="18")
...
merit_avg <-merit_aug %>% filter(month==2) %>% mutate(percentile=round(merit/hourly_avail*100))%>%
  #filter(offer_sum!="TRADE")%>%
  group_by(year,percentile,on_peak) %>%
  summarize(n=n(),p_mean=mean(price),min_price=min(price),max_price=max(price),
            ghg_mean=mean(merit_ghgs),min_ghg=min(merit_ghgs),max_ghg=max(merit_ghgs))
