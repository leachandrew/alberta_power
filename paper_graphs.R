...
#           ymin = -Inf, ymax = Inf)+
 # annotate("rect", fill = "grey70", alpha = .3, 
  ...
  gen_fuel <- df2 %>% mutate(Plant_Type=factor(Plant_Type,levels=AB_plant_order))%>%
    ...
    NULL
  gen_fuel
  ggsave(file="gen_fuel.png", width = 12, height = 7,dpi = 300)
  ...
  capacity_fuel <- df2 %>% mutate(Plant_Type=factor(Plant_Type,levels=AB_plant_order))%>%
    ...
    NULL
  capacity_fuel
  ggsave(file="capacity_fuel.png", width = 12, height = 7,dpi = 300)
...
aeso_key<-"eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ6djBkdDQiLCJpYXQiOjE2MTcxMzk3MTh9.2ecqLByX-zVFeL29nv_mjzfqChZLwPcHEo0qxZNJ6vc"


start_date<-Sys.Date()-days(5)
end_date<-Sys.Date()-months(1)

GET(url="https://api.aeso.ca/report/v1/poolPrice",
    #add_headers(X-API-Key= aeso_key),
    add_headers("X-API-Key" = aeso_key,"accept"= "application/json"),
    query = list(
...
start_time<-Sys.time()
fct<-get_forecast_report(Sys.Date(),end_date)
print(paste("Elapsed time is",time_length(interval(start_time, Sys.time())*12, "seconds"),"seconds"))
