...
#constructing the synthetic plants based on plant type

synth_plants<-function(){
  ...
  merit_aug<-merit_data%>% #filter(year(date)>=2015) %>% #testing just with 2019 data
    ...
    clean_merit_trade() %>% ungroup() %>% select(-merit) %>% filter(import_export!="E")
  ...
  #old synth plant stuff from here
  #memory tag <- merit_store<-merit_aug  
  ...
  group_by(date,he,Plant_Type) %>%
    arrange(price,Plant_Type) %>% mutate(type_merit=cumsum(available_mw),
    ...
    ungroup()%>%arrange(date,he,Plant_Type,type_merit)
 
    
    #memory tag <- merit_store<-merit_aug  
    
  #convert all blocks to percentile blocks
  build<-1
  if(build==1){
    merit_bids<-merit_aug %>% group_by(date,he,Plant_Type)%>% 
    ...
    merit_bids<-merit_bids%>% group_by(date,he,Plant_Type) %>%
      ...
      ungroup() %>% select(-from,-price,-step_func,-co2_est,#-ghg_func,-ctax_func,
                           -net,-ctax_cost)
    print(paste("Build bids, cleaned data frame, elapsed time is",time_length(interval(start_time, Sys.time()), "seconds"),"seconds"))
    save(merit_bids,file= "synth_bids.RData")
    }
  #bids data has date, he, asset_id, and bid_10 through bid_100 for all data.
  if(build!=1){
    load(file = "synth_bids.RData") 
  }
  
  #melt and match
  merit_aug<-merit_bids %>% select(date,he,Plant_Type,grep("bid_",names(merit_bids)))%>%
    melt(id=c("date","he","Plant_Type"),variable.name="percentile",value.name = "price")%>%
  ...
  #and you're done
  save(merit_aug, file="synth_ghg_price.Rdata" ) 
...
merit_avg <-merit_aug %>% group_by(Plant_Type, year, percentile) %>%
  ...
  filter(!Plant_Type %in% c("IMPORT","EXPORT","SOLAR","WIND","OTHER") )
my_palette<-colors_tableau10()[c(8,4,1,6,2)]



set_png("synth_plants.png",height = 1000)
ggplot(filter(merit_avg,year>=2014))+#geom_line(aes(percentile,price,color=Plant_Type,group=Plant_Type))+
  geom_ribbon(aes(x=percentile,ymin=min_price, ymax=max_price,fill=Plant_Type,group=Plant_Type),alpha=.3)+
  geom_line(aes(x=percentile,y=p_mean,colour=Plant_Type,group=Plant_Type),size=2)+
  facet_grid(Plant_Type~year)+
...
scale_x_continuous(expand=c(0,0),limits = c(0,100), breaks = c(0,25,50,75,100),labels = c(0,25,50,75,100))+
  ...
  labs(x=paste("Offered Generation (% of Capacity)"),y="Price ($/MWh)",
       title=paste("Synthetic Plants Average Offer Schedules and Ranges",sep = ""),
...
merit_aug<-merit_aug %>%
  mutate(policy=case_when(year(date)<2016 ~ "SGER 1.0",
                          ...
                          year(date)==2019 ~ "CCIR",
                          TRUE ~ "TIER"),
         policy=factor(policy,levels=c("SGER 1.0","SGER 2.0","SGER 3.0","CCIR","TIER"))
  )
...
merit_aug<-merit_aug %>% #select(-nit_settle_cad_gj) %>%
  left_join(ngx_data_read(),by=c("date"))
#carry forward missing gas price data
merit_aug <-merit_aug %>% mutate(nit_settle_cad_gj=na.locf(nit_settle_cad_gj))
...
by_type <- merit_aug %>%  mutate(p2=percentile^2,p3=percentile^3,p4=percentile^4,gas2=nit_settle_cad_gj^2,gas3=nit_settle_cad_gj^3,
...
test<-by_type %>% 
  unnest(tidied, .drop = TRUE)  %>% filter(grepl("olicy",term),Plant_Type %in% c("COAL","NGCC"))
...
set_png("offer_changes.png")
ggplot(filter(test,percentile!=100))+#geom_line(aes(percentile,price,color=Plant_Type,group=Plant_Type))+
  geom_errorbar(aes(x=percentile,ymax=upper_ci, ymin=lower_ci,group=Plant_Type,color=Plant_Type),size=2)+
  geom_point(aes(x=percentile,y=estimate,colour=Plant_Type,group=Plant_Type),size=2)+
  facet_grid(~term)+
 scale_x_continuous(expand=c(0,0),limits = c(0,100), breaks = c(0,25,50,75,100),labels = c(0,25,50,75,100))+
  scale_y_continuous(expand=c(0,0))+
  scale_fill_manual("",values=my_palette)+
...
gc()
# Split into pieces, fit model to each piece
by_type <- merit_aug %>%  filter(date<=ymd("2020-03-01"))%>% #take out the COVID times
                                   mutate(p2=percentile^2,p3=percentile^3,p4=percentile^4,gas2=nit_settle_cad_gj^2,gas3=nit_settle_cad_gj^3,
                                 year_fac=as.factor(year), yearmonth=interaction(month_fac,year_fac),
                                 tight=(supply_cushion<=500),really_tight=(supply_cushion<200),he_fac=as.factor(he),
                                 hdd_yeg2=hdd_yeg^2,cdd_yeg2=cdd_yeg^2,
                                 hdd_yyc2=hdd_yyc^2,cdd_yyc2=cdd_yyc^2,
                                 hdd_ymm2=hdd_ymm^2,cdd_ymm2=cdd_ymm^2,
                                 )%>%
  #nest(-c(Plant_Type,percentile)) %>%
  nest(-c(Plant_Type,percentile)) %>% 
  mutate(
...
test$term<-factor(test$term)
levels(test$term)<-factor(test$term)
levels(test$term)<-c("SGER 2.0","SGER 3.0","CCIR/TIER")
