...
asset_fuel <- read.table('C:/Users/safton/Documents/R/Merit Scripts/Completed Plants.txt', header = TRUE)
...
data_proc_adder<-function(data_sent,max_cap){    
  ...
  for(date_id in unique(data_sent$date)) {
    ...
    if(nrow(df_test)>1){
      step_fun<-generate_func(df_test$from[-1],df_test$adder)
      df2[,date_count]<-step_fun(df2$MW)
      names(df2)[date_count]<-as.character(df_test$date[1])
    }
    else{
      df2[,date_count]<-df_test$adder
      names(df2)[date_count]<-as.character(df_test$date[1])
    }
...
#Add fuel price table into fuel table and calculate bid adder
fuel_table <- merge(fuel_table, fuel_price)
fuel_table$adder <- pmax(fuel_table$price - fuel_table$fuel_cost,0)
  ...
  tryCatch({  
    ...
    {  
      ...
      for(input_hour in 1:168)
      {
        ...
        data_store[input_hour]<-data.frame(c(input_hour,data_proc_adder(plant_year,max_cap)))
      }
