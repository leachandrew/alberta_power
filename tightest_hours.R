...
#open AESO workbook
supply_cushion<-function(){
    data_read<-read_excel("AESO-Supply-Cushion.xlsx",sheet="All Hours (5 Years)")
  data_read
}
...
hours<-hours%>%left_join(forecast_data,by=c("local_date"="time"))
merit_hour<-merit_aug %>% group_by(time) %>% summarize(supply_cushion=mean(supply_cushion))
hours<-hours%>%left_join(merit_hour,by=c("local_date"="time"))


set_png("tight_hours_merit.png")
ggplot(hours)+
  geom_point(aes(actual_ail,supply_cushion,color=capacity_year),size=2)+
  geom_smooth(aes(actual_ail,supply_cushion,group=capacity_year,color=capacity_year,
                  fill=capacity_year),method='lm')+
  scale_color_manual("Capacity Year",values=colors_tableau10())+
  scale_fill_manual("Capacity Year",values=colors_tableau10_light())+
  theme(legend.position = "bottom")+ajl_line()+
  labs(x="Hourly Internal Load (MW)",y="Hourly Supply Cushion (MW)",
       subtitle="Points are 250 tightest hours in the Alberta market (2012-2017)",
       title="Alberta Supply Cushions and Load (2012-2017)",
       caption="Source: AESO Data, graph by Andrew Leach")
dev.off()

ggplot(hours)+
  geom_point(aes(supply_cushion_mw,supply_cushion,color=capacity_year),size=2)+
  geom_line(aes(supply_cushion_mw,supply_cushion_mw),size=2,color="black")+
  facet_grid(~capacity_year)
  

set_png("tight_hours_ail.png")
ggplot(hours)+
  geom_point(aes(actual_ail,supply_cushion_mw,color=capacity_year),size=2)+
  geom_smooth(aes(actual_ail,supply_cushion_mw,group=capacity_year,color=capacity_year,
                  fill=capacity_year),method='lm')+
  scale_color_manual("Capacity Year",values=colors_tableau10())+
  scale_fill_manual("Capacity Year",values=colors_tableau10_light())+
  theme(legend.position = "bottom")+ajl_line()+
...
merit_combine<-function(merit_AS_sent,forecast_sent,start_date=as.Date("2001-01-01"),end_date=as.Date("2001-01-01")){
  ...
  merit_AS_sent<-merit_AS_sent %>% 
    group_by(date,he,time,product,actual_ail,actual_posted_pool_price) %>% 
    summarize(active_price=sum(price_mw*volume_mw)/sum(volume_mw)+mean(actual_posted_pool_price),
              mean_standby_price=mean(premium_price_mw),
              min_standby_price=min(premium_price_mw),
              max_standby_price=max(premium_price_mw)) %>%
    ungroup() 
...
aeso_sc_data<-supply_cushion()
merit_data_aug<-augment_data(merit_data)
